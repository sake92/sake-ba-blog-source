...
object HelloWorld extends JavaTemplate {
  ...
  def introSection = Section(
    ...
    div(
      ...
      """.md,
      img(
        src    := files.images.java.`compiler.gif`.ref,
        width  := 584,
        height := 138,
        alt    := "Kompajliranje i pokretanje Java programa"
      )
    )
  )
  ...
  def compileAndRunSection = Section(
    ...
    div(
      ...
      Kucamo sljedeće:
      ```sh
      C:\programiranje> javac PoyySvijeteApp.java
      C:\programiranje> java PoyySvijeteApp
      Poyy sviete!
      ```
